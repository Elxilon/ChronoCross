...
class Database
{
    ...
    private PDO $pdo;
    public function __construct($db_name='chrono', $db_host='127.0.0.1', $db_port='3306', $db_user = 'root', $db_pwd='root') {
    ...
    private function getCoureursFromCourse(int $id): array {
        ...
        return $this->exec("SELECT CONCAT(UPPER(utilisateurs.nom), ' ', utilisateurs.prenom) AS nom_prenom, Temps, Classement
            FROM course_has_coureur AS chc
            INNER JOIN utilisateurs on chc.idUtilisateur = utilisateurs.id
            WHERE Course_idCourse = :id
            ORDER BY Classement ASC", ['id' => $id]);
    }
    ...
    public function getAllCourses(): array {
        ...
        foreach ($courses as $key => $course) {
            // on utilise $courses (puis donc $key pour identifier la course) pour persister la liste en dehors de la boucle
    ...
    /**
     * @param string $query la requête qui va être envoyer à la bdd
     * @param array|null $params les paramètres s'il y en a, c'est le cas pour getCoureursFromCourse
     * @return array Tableau contenant le(s) résultat(s) de la requête passée en paramètre de la fonction
     */
